...
\textbf{Positional Embeddings} inject positional information into the embedding vector, allowing the transformer to distinguish token order when applying \textbf{Self-Attention}.
...
\begin{itemize}
    ...
    {\setlength{\jot}{1em}
    \begin{align}
        \mathrm{PE}(pos, 2i) = \mathrm{sin} \left(\frac{p}{10000^{2i / d_{\mathrm{model}}}}\right) \\
        \mathrm{PE}(pos, 2i + 1) = \mathrm{cos} \left(\frac{p}{10000^{2i / d_{\mathrm{model}}}}\right)
    \end{align}
    }
\end{itemize}
...
\begin{align}
    \mathrm{GATED\_FFN}_{\phi}(u) = W_o \mathrm{GLU}_{\phi}(u) + b_o
\end{align}
...
Since most operations in the transformer rely on matrix multiplication, they can be efficiently computed in parallel on modern GPUs and other specialized hardware (i.e.\ AWS Trainium\footnote{\url{https://aws.amazon.com/ai/machine-learning/trainium/}}).
...
\textbf{QLoRA} is a further improvement over \textbf{LoRA} that uses a quantized \footnote{Technique where the model's weights are converted to lower-precision representation to reduce memory usage.} version of the pretrained model and memory paging which further reduces the memory overhead.
...
\textbf{PyTorch}\footnote{\url{https://pytorch.org/}} is an open-source Python machine learning framework, originally developed by Meta, that is mainly designed for deep learning, including computer vision and natural language processing.
...
\textbf{HuggingFace}\footnote{\url{https://huggingface.co}} is an open-source platform and community focused on machine learning.
...
\textbf{HuggingFace Hub}\footnote{\url{https://huggingface.co/docs/hub/index}} is an online platform that allows the hosting of git repositories, models, datasets and hosting of demo apps (spaces), additionally to hosting, it also supports discussion and collaboration of users.
...
\textbf{Transformers}\footnote{\url{https://huggingface.co/docs/transformers/en/index}} is a library that contains many useful tools for interfacing with different machine learning models, providing a unified framework for both training and inference.
...
\textbf{PEFT}\footnote{\url{https://huggingface.co/docs/peft/index}} (Parameter-Efficient Fine-Tunig) is a library that implements efficient fine-tuning mechanisms for large pretrained models, such as \textbf{LoRA} and \textbf{QLoRA}.
