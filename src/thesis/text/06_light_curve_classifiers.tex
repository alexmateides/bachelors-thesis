...
We have also experimented with $k$-fold \textbf{cross-validation}, however its implementation using the transformers library proved cumbersome, the training time has grown significantly ($\approx k\times$) and the performance wasn't noticeably better when compared to the alternative train/val/test protocol.
...
\begin{figure}[!htbp]
    ...
    \caption[Segment vs. TIC classification]{
        Example of segment-level and TIC-level classification -- the model predicts labels of 3 segments of an eclipsing variable.
        One of the segments is misclassified as non-variable because the secondary eclipse isn't clear enough, however the other two segments are predicted correctly.
        After aggregation, the final predicted label for the whole object is correct, improving the overall classification performance.
        Note that the TIC-level prediction isn’t done by majority voting, but rather by averaging logits}
    \label{fig:06_segment_vs_tic_classification}
\end{figure}

\subsubsection{Segment \& Class Weighted Loss}
...
\begin{align}
    \mathcal{L} = \frac{\sum_{i=1}^{B} s_i \, L_{\mathrm{WCE}}^{(i)}}{\sum_{i=1}^{B} s_i}, \label{eq:06-turbo-loss}
\end{align}

where $B$ is the batch size and $L_{\mathrm{WCE}}^{(i)}$ is the \hyperref[eq:04-weighted-cross-entropy]{Weighted Loss (4.12)} for segment $i$.
...
\dirtree{%
    ...
    .2 driver.py \DTcomment{Driver code}.
...
Additionally, the trainer can continuously report the training progress to a Weights \& Biases\footnote{\url{https://wandb.ai/}} dashboard thas is accessible from the internet, allowing the user to remotely monitor long-running experiments.
...
Chronos-2\footnote{\url{https://huggingface.co/amazon/chronos-2}} is a 120M-parameter, encoder-only time series foundation model developed by Amazon.
...
We created a relatively simple baseline model by replacing the transformer blocks with a CNN, the usage of the same general multi-channel architecture and training techniques make the baseline CNN more comparable to the proposed transformer-based models.

The CNN was implemented with two 1-dimensional convolutional layers with a kernel of size 3 in each of them, all channels and the extra feature MLP were enabled for the model.

\subsection{Results}

Even this simple baseline CNN performed strongly and achieved good results.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{X c@{\hspace{3em}}c}
        \toprule
        \textbf{Task} & \textbf{Macro F1}  & \textbf{TIC Macro F1}  \tabularnewline \midrule
        \textbf{A} & 0.0.8893 & 0.9061 \tabularnewline
        \textbf{B} & 0.8819 & 0.9232 \tabularnewline
        \bottomrule
    \end{tabularx}
    \caption[Baseline Model Validation Metrics]{Baseline Model Validation Metrics.}
    \label{tab:06_baseline_model_performance}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Loss Function.} The proposed \hyperref[eq:06-turbo-loss]{Segment \& Class Weighted Loss (6.2)} was used.
\end{itemize}
